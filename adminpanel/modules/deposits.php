...
if($_GET['action'] == "add_depo") {
	...
	if(!$name || !$sum || !$plan || !$paysys) {
		...
		$sql = "INSERT INTO `deposits` (username, user_id, date, plan, sum, paysys) VALUES ('".$name."', ".$name_id.", ".time().", ".$plan.", ".$sum.", ".$paysys.")";
		mysql_query($sql);
	...
	if (!$themes) {
		...
		$row2		= mysql_fetch_array($result2);
			...
			print "<tr>
                  ...
                  <td><a href=\"?a=edit_user&id=".$row['user_id']."\"><b>".$row['username']."</b></a></td>
                  <td>".$row['sum']."</td>
                  <td>";
				  if($row['paysys'] == 1) { print 'PerfectMoney'; } else { print 'LibertyReserve'; }
			print "</td>
				  <td>".$row2['name']."</td>
                </tr>";
		}
...
$sql = "SELECT * FROM deposits WHERE status = 0 ".$su." ".$sort;
...
<table width="100%" border="0">
	<tr>
		...
		<td align="right"><input class="input" style="width: 550px;" type="text" name="name" size="93" /></td>
	</tr>
	<tr>
		<td><strong>Сумма:</strong></td>
		<td align="right"><input class="input" style="width: 550px;" type="text" name="sum" size="93" value="100.00" /></td>
	</tr>
	<tr>
		<td><strong>Тарифный план:</strong></td>
		<td align="right"><select class="input" name="plan" style="width: 550px;">
<?php
...
?></select>
	...
	<tr>
		...
		<td align="right"><select class="input" name="paysys" style="width: 550px;">
<?php
if($cfgPerfect) {
	print '<option value="1">PerfectMoney</option>';
}
if($cfgLiberty) {
	print '<option value="2">LibertyReserve</option>';
}
?>
		</select>
	</td>
...
<table width="100%" border="0">
	<tr>
		...
		<td><input class="inp" style="background-color: #ffffff; width: 500px;" type="text" name="percent" size="93" /></td>
